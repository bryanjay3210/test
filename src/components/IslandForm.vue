  ...
  <v-row>
    ...
    >
      <v-card>
        ...
        <v-container>
          <v-form>
            <v-card-title> BloodType Information </v-card-title>
            <v-row>
              ...
              <v-col cols="10" xs="10" sm="4" md="4" lg="3" xl="3">
                <v-checkbox v-model="isactive" label="IsActive?"></v-checkbox>
              </v-col>
              <v-col cols="12" xs="12" sm="4" md="4" lg="3" xl="3">
                <v-btn color="error" block @click="confirmed">Save</v-btn>
              </v-col>
            </v-row>
          </v-form>
        </v-container>
...
export default {
  data: () => ({
    ...
    rules: [
      ...
      (value) => (value && value.length >= 1) || "Min 2 characters",
    ],
    // religions: [
  ...
  methods: {
      showModalBloodType(form) {
      this.id = form.id, 
      this.religion = form.bloodtype, 
